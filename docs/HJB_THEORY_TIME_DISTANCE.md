...
This note documents the HJB term used in the current implementation. The former
LiDAR-sector state extension has been removed; LiDAR is still used by the costmap
and environment safety rewards, but not by the HJB residual or `vec` observation.
