...
class PeopleManager:
    ...
    @staticmethod
    def _configure_people_settings(cfg=None) -> None:
        import carb
    ...
    @staticmethod
    def _ensure_navmesh_volume() -> None:
        ...
        if len(nav_volume_paths) == 0:
            # Place under env_0 so bake uses same coordinate space as warehouse (GeneratedScene)
            parent = "/World/envs/env_0" if stage.GetPrimAtPath("/World/envs/env_0").IsValid() else "/World"
        ...
        # Scale the first volume to cover warehouse floor area. Z=80 ensures floor is included
        # (warehouse_with_forklifts and similar assets can have floor at various Z).
        if len(nav_volume_paths) > 0:
            xform = Gf.Matrix4d(1.0)
            xform.SetScale(Gf.Vec3d(80.0, 80.0, 80.0))
            omni.kit.commands.execute(
                "TransformPrim",
                path=Sdf.Path(nav_volume_paths[0]),
    ...
    @staticmethod
    def _ensure_command_file() -> str | None:
        ...
        try:
            ...
            with open(fd, "w") as f:
                for i in range(6):
                    name = "Character" if i == 0 else f"Character_0{i}"
    ...
    def setup(self) -> None:
        ...
        self._configure_people_settings(self.cfg)
        ...
        max_people = int(getattr(self.cfg, "people_slot_count", 0))
        if max_people <= 0:
            return
        ...
        for i in range(max_people):
            name = CharacterUtil.get_character_name_by_index(i)
    ...
    def _enforce_z_override(self, env_origins: torch.Tensor) -> None:
        ...
        root_lift = float(getattr(self.cfg, "people_character_root_z_lift", 0.95))
        target_z = floor_z + root_lift
    ...
    def _inject_new_commands(self, env_origins: torch.Tensor) -> None:
        ...
        for slot_idx in self._active_slots:
            name = self._agent_names[slot_idx]
    ...
    def reset(self, env_ids: Sequence[int] | torch.Tensor, env_origins: torch.Tensor) -> None:
        ...
        env_id = int(env_ids_list[0])
        origin = env_origins[env_id]
        floor_z = float(getattr(self.cfg, "people_floor_z_world", 0.0))
        root_lift = float(getattr(self.cfg, "people_character_root_z_lift", 0.95))
        spawn_z = floor_z + root_lift

        min_count, max_count = getattr(self.cfg, "people_count_range", (0, 0))
        max_count = min(int(max_count), len(self._root_paths))
        min_count = min(int(min_count), max_count)
        if max_count <= 0:
            return

        active_count = int(torch.randint(min_count, max_count + 1, (1,), device=self.device).item())
        perm = torch.randperm(len(self._root_paths), device=self.device).tolist()
        requested_active_slots = set(perm[:active_count])
        ...
        identity_quat = (1.0, 0.0, 0.0, 0.0)
        for idx in range(len(self._root_paths)):
            self._set_character_pose(idx, hidden_pose, identity_quat)
            self._set_visibility(self._root_paths[idx], False)
            if idx < len(self._control_paths):
                self._set_visibility(self._control_paths[idx], False)
            self._set_subtree_visibility(self._root_paths[idx], False)

        for slot_idx in requested_active_slots:
            sample_ok = False
